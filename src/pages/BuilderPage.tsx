...
export default function BuilderPage() {
  ...
  const weeklyStats = useMemo(() => {
    ...
    let exercises = 0;
    recent.forEach((s) => {
      Object.values(s.sets).forEach((list) => {
        exercises += 1;
  ...
  const records = useMemo(() => {
    ...
    data.sessions.forEach((s) => {
      Object.values(s.sets).forEach((list) => list.forEach((set) => {
  ...
  const volumeByDay = useMemo(() => data.sessions.map((s) => ({
    ...
    volume: Object.values(s.sets).flat().reduce((a, b) => a + b.weight * b.reps, 0),
    sets: Object.values(s.sets).flat().filter((x) => x.completed).length
  })), [data.sessions]);
